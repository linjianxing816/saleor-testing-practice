...
def test_checkout_create_should_success(saleor_client):
    ...
    # 第 3 步：根据 channel_slug 查询 products，提取 product_id
    ...
    # 第 4 步：根据 product_id 查询 variants，提取 variant_id
    ...
    # 第 5 步：使用 variant_id 创建 checkout
    checkout_response = saleor_client.graphql(
    ...
    print("checkout 创建返回结果 =", checkout_data)

    assert "errors" not in checkout_data

    checkout_create_data = checkout_data["data"]["checkoutCreate"]

    assert checkout_create_data["errors"] == []
    assert checkout_create_data["checkout"] is not None

    checkout = checkout_create_data["checkout"]

    assert checkout["id"] is not None
    assert checkout["id"] != ""

    assert checkout["token"] is not None
    assert checkout["token"] != ""

    assert checkout["lines"] is not None
    assert len(checkout["lines"]) > 0

    first_line = checkout["lines"][0]

    assert first_line["quantity"] == 1
    assert first_line["variant"]["id"] == variant_id

    print("创建成功的 checkout_id =", checkout["id"])
    print("创建成功的 checkout_token =", checkout["token"])